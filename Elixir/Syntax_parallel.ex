# Syntax highlighter - Parallel version
#
# Carlos Enrique Rosete Pascual
# 2026-06-11
#

defmodule TecLexer.Parallel do
  ...
  # file found inside it, spawning one Task per file so all cores are used.
  # Prints the total wall-clock time when done.
  def run(directory, output_dir \\ "output_parallel") do
    # Create the output directory if it does not exist yet
    File.mkdir_p!(output_dir)
    {time, _} = :timer.tc(fn ->
      process_directory(directory, output_dir)
    end)
    IO.puts("PARALLEL   | Directory: #{directory} | Time: #{time / 1_000_000} s")
  end

  # List all .ex and .exs files in the directory and process them in parallel.
  # Task.async_stream creates one Task per file and distributes them across
  # all available CPU cores automatically -- no manual thread count needed.
  defp process_directory(directory, output_dir) do
    ...
    |> Task.async_stream(
         fn name -> highlight_file(Path.join(directory, name), output_dir) end,
         # Use as many concurrent tasks as there are schedulers (one per core)
         max_concurrency: System.schedulers_online(),
         # Never time out -- large files may take a while
         timeout: :infinity
       )
    |> Stream.run()
  end
  ...
  # Each call runs inside its own Task so multiple files are processed at once.
  defp highlight_file(path, output_dir) do
    ...
    # Build the output filename: <basename>_parallel.html
    base = Path.basename(path, Path.extname(path))
    out  = Path.join(output_dir, "#{base}_parallel.html")
